...
export default class UserDBService {
  ...
  static async getDriverRating(driverId) {
    return UserModel.findAndCountAll({
      where: {
        id: driverId
      },      
      include: [
        {
          model: TripModel,
          attributes: [ 'driverId','ratingDriver', 'status','totalKms','totalTimeInMinute'],
          as: 'driverData'
        }
      ],
      nest: true
  ...
  static getAdmin() {
    return UserModel.findAll({
      ...
      attributes: ['uuid', 'fullName', 'loginId', 'id', 'profileImage'],
      order: [
        ['id', 'DESC'],
      ],
      raw: true,
