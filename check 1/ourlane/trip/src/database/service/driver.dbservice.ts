...
export default class DriverDBService extends BaseDbService {
  ...
  static async getNearestCab(options: {
    ...
    const startTime = moment(options.dateTime).add(-10, 'minutes').utc().format();
    const endDateTime = moment(options.dateTime).utc().format();
    const { latitude, longitude,carTypeId } = options;
    const result = await sequelize.query(`SELECT u.fullName as driverName,
      ...
      ct.pricePerHour,ct.pricePerHourKM, ct.pricePerMin,ct.carTypeName,
      c.carNumber,c.carModel,d.isOnlineStatus as driverStatus,
      c.carCapacity, ct.carImage, ct.bookingFee, c.addDescription,
      p.distance_unit
      ...
      JOIN (
       ...
       inner join cartype ct on ct.id = c.carTypeId 
       WHERE d.isOnlineStatus = 'Available' and c.status='Active' and ct.id = ${carTypeId} and ct.status='Active' AND (fromDateTime > :endDateTime OR toDateTime < :startTime)
              AND p.distance_unit
               * DEGREES(ACOS(COS(RADIANS(p.latpoint))
               * COS(RADIANS(d.addressLatitude))
               * COS(RADIANS(p.longpoint) - RADIANS(d.addressLongitude))
      ...
      ORDER BY distanceInKm`, { replacements: { inputLat: latitude, inputLng: longitude, kilometer: kilometer, startTime: startTime, endDateTime: endDateTime }, type: QueryTypes.SELECT });
    ...
    await result.forEach(async e => {
      const param = {
        ...
        price_kilometer: e.pricePerHourKM,
        price_perHour : e.pricePerHour,
        price_perminute: e.pricePerMin,
        booking_fee: e.bookingFee,
        booked_Hours: options.bookedHours ? options.bookedHours : null
      };
      ...
      if (options.bookedHours) {
        e.bookedHours = options.bookedHours
      }
    });
    return result;
  }
  ...
  static fareCalculation(options: {
    ...
    booking_fee: Float,
    price_perHour?: Float,
    booked_Hours?: number, // only for hourly booking
    waitingTime?: number,
    waitingChargePerMin?: number
  }) {
    if (options.booked_Hours) {
      // Formula : Booking Fees + (booked hours * price per hour) + (waitingTime * waitingCharges)
      const primaryCharge = options.booked_Hours * options.price_perHour;
      const waitingCharge = (options.waitingTime || 0) * (options.waitingChargePerMin || 0);
      const totalFare = options.booking_fee + primaryCharge + waitingCharge;
      return {totalFare, waitingCharge};
    } else {
      // Formula : Booking Fees + ETA min * Price per min + ETA km * Price per km + (waitingTime * waitingCharges)
      ...
      const waitingCharge = (options.waitingTime || 0) * (options.waitingChargePerMin || 0);
      const totalFare = (options.booking_fee + timeAmount + kilometerAmount + waitingCharge);
      return {totalFare, waitingCharge};
    }
  }
  ...
  static async getDriverTrips(options: {
    id: string
  }) {
  ...
  static async getDriverCars(options: {
    ...
    return UserModel.findAndCountAll({
      where: options,
      attributes:['fullName','mobileNumber','role','id'],
      include: [{
        model: CarModel,
