...
export default class TokenHandler {
  ...
  //verify IO token for mqtt
  static async verifyIoMQTT(mqttObject) {
    if (mqttObject.token) {
      return jwt.verify(mqttObject.token, jwtSecret, async (err: Error, decoded: any) => {
        if (err) {
          if (err.name === 'TokenExpiredError') {
            return (new CustomError({ serviceName, ...TOKEN_EXPIRED }));
          } if (err.name === 'JsonWebTokenError') {
            return (new CustomError(TOKEN_UNAUTHORIZED));
          }
          return (new CustomError({ serviceName, ...TOKEN_UNAUTHORIZED }));
        }
        if (decoded) {
          mqttObject['decoded'] = decoded;
          if (decoded.role == null || decoded.role === undefined) {
            return (new CustomError({ serviceName, ...ACCESS_DENIED }));
          }
          return mqttObject
        }
        return (new CustomError({ serviceName, ...TOKEN_EXPIRED }));
    ...
    return (new CustomError({ serviceName, ...NO_TOKEN }));
  }
}
